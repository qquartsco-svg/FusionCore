...
> **정체성**: D-T 핵융합 반응에서 전력(MW)과 추력(N)을 뽑아내는 **범용 플라즈마 추진 런타임**.
> 토카막 공학 제어 핵융합 전용. 어떤 우주선에든 pluggable.
...
```
전기추진 모드 (Electric Ion):
  v_e = Isp × g₀               [m/s],  g₀ = 9.80665
  F   = 2 × P_thrust / v_e     [N]
  Isp ≈ 3,000 ~ 10,000 s

직접추진 모드 (Direct Fusion Drive):
  v_e = Isp × g₀               [m/s]
  F   = ṁ × v_e                [N]
  Isp ≈ 10,000 ~ 20,000 s
...
| 레이어 | 현재 | 확장 방향 |
|--------|------|-----------|
| 반응 | D-T / D-He3 / D-D / p-B11 NRL 근사 | 정밀 핵단면적 데이터 교체 |
| 플라즈마 | Euler 적분 | MHD 솔버 / ITER 파라미터 |
| 열 | Stefan-Boltzmann 복사 | 다층 방열판 / 열파이프 |
| 차폐 | Beer-Lambert 단순 | 몬테카를로 중성자 수송 |
| 추진 | 전기/직접/하이브리드 | VASIMR / 펄스 핵융합 |
| 안전 | Ω 5레이어 | ML 이상 탐지 |
